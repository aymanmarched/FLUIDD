...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-soft p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <div class="text-sm text-slate-500">Mes commandes</div>
                <h2 class="text-2xl md:text-3xl font-extrabold tracking-tight">Mes commandes d'entretien</h2>
                <p class="text-slate-600 mt-1">Consultez, modifiez et payez vos commandes.</p>
            </div>

            <a href="{{ url('/service/entretien/entretenir-ma-maison') }}" target="_blank"
        ...
        <div class="bg-white border border-dashed border-gray-200 rounded-2xl p-10 text-center">
            <div class="mx-auto h-12 w-12 rounded-2xl bg-sky-100 text-sky-700 flex items-center justify-center mb-3">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6M9 8h6M5 3h14a2 2 0 0 1 2 2v16a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2Z"/>
                </svg>
            </div>
            <div class="font-extrabold text-lg">Aucune commande d'entretien</div>
            <p class="text-slate-500 mt-1">Créez votre première commande en cliquant sur “Nouvelle commande d’entretien”.</p>
        </div>
    @else
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach($commandes as $cmd)
                            ...
                            <tr class="hover:bg-gray-50/60">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm border {{ $badge }}">
                                        {{ $cmd->status_label }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    @if($cmd->is_paid)
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm border bg-emerald-50 text-emerald-700 border-emerald-100">
                                            Payée
                                        </span>
                                    @else
                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm border bg-rose-50 text-rose-700 border-rose-100">
                                            Non payée
                                        </span>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-2 justify-center">
                                        <a href="{{ route('client.entretiens.show', $cmd->reference) }}"
                                        ...
                                        @if($cmd->can_edit)
                                            <a href="{{ route('client.entretiens.edit', $cmd->reference) }}"
                                        ...
                                        @if(!$cmd->is_paid && !$cmd->annule)
                                            <a href="{{ route('client.payments.create', ['type' => 'entretien', 'reference' => $cmd->reference]) }}"
                                               class="inline-flex items-center justify-center px-4 h-10 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold shadow-soft transition">
                                                Payer
                                                @if(($cmd->pay_discount_percent ?? 0) > 0)
                                                    <span class="ml-2 text-xs bg-white/20 px-2 py-0.5 rounded-full">
                                                        -{{ $cmd->pay_discount_percent }}%
                                                    </span>
                                                @endif
                                            </a>
                                        @endif
                    ...
                    <div class="p-4">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <div class="shrink-0 flex flex-col items-end gap-2">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs border {{ $badge }}">
                                    {{ $cmd->status_label }}
                                </span>

                                @if($cmd->is_paid)
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs border bg-emerald-50 text-emerald-700 border-emerald-100">
                                        Payée
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs border bg-rose-50 text-rose-700 border-rose-100">
                                        Non payée
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="mt-4 flex flex-wrap gap-2">
                            ...
                            @if(!$cmd->is_paid && !$cmd->annule)
                                <a href="{{ route('client.payments.create', ['type' => 'entretien', 'reference' => $cmd->reference]) }}"
                                   class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold shadow-soft transition w-full">
                                    Payer
                                    @if(($cmd->pay_discount_percent ?? 0) > 0)
