...
<section class="w-full bg-gradient-to-br from-accent to-[#b4bfca] px-4 py-6 md:px-8 md:py-8">
    <div class="mx-auto max-w-3xl">
        <div class="rounded-2xl border border-white/60 bg-white/85 p-5 shadow-xl backdrop-blur md:p-6">
            ...
            <form method="POST"
                  action="{{ route('service.remplacer.step5.store', $client->id) }}"
                  id="reservationForm">
                @csrf
                ...
                <div id="hoursContainer" class="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
...
datePicker.addEventListener('change', async () => {
    ...
    try {
        const response = await axios.post('{{ route('service.remplacer.getAvailableHours') }}', {
            date: date
        });
