    ...
    <div class="mx-auto max-w-xl">
        <div class="rounded-2xl border border-white/60 bg-white/85 p-5 shadow-xl backdrop-blur md:p-6">
            ...
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 text-center">
                <h3 class="text-lg font-extrabold text-slate-900">Entrez le code reçu par SMS</h3>
                <p class="mt-2 text-sm leading-6 text-slate-600">
                    Saisissez le code pour continuer.
                </p>
            </div>

            <form method="POST" action="{{ route('service.remplacer.step4.verify') }}" class="mt-5">
                @csrf
            ...
            <form method="POST" action="{{ route('service.remplacer.resendSms') }}" class="mt-3">
                @csrf
                <input type="hidden" name="client_id" value="{{ $client->id }}">
                <input type="hidden" name="reference" value="{{ $reference }}">
