{{-- resources/views/admin/clientsentretien/index.blade.php --}}
@extends('admin.layout')

@section('page_title', "Commandes d'entretien")
        ...
        <div class="flex items-start sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 flex items-center gap-3">
                    ...
                    Liste des Commandes d'entretien
                </h1>
                <p class="text-slate-500 text-sm mt-1">Suivi des commandes et réservations.</p>
            </div>
        </div>
    </div>
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-slate-50 border-b border-slate-200">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left font-semibold text-slate-600">Réservation</th>
            ...
            <tbody class="divide-y divide-slate-200">
                @foreach($commandes as $cmd)
                    @php $client = $cmd->client; @endphp
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-6 py-4 text-slate-900 font-semibold">{{ $cmd->reference }}</td>
                        ...
                        <td class="px-6 py-4 text-slate-500">
                            {{ optional($cmd->reservation)->date_souhaite ?? '—' }}
                        </td>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        ...
                        <div class="text-base font-extrabold text-slate-900 break-words">{{ $cmd->reference }}</div>
                ...
                <div class="mt-3 grid grid-cols-1 gap-2 text-sm">
                    ...
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <span class="text-slate-800 font-semibold">{{ $client?->created_at?->format('d/m/Y') ?? '-' }}</span>
                    </div>

                    <div class="flex items-center justify-between gap-3">
                        <span class="text-slate-500">Réservation</span>
                        <span class="text-slate-800 font-semibold">{{ optional($cmd->reservation)->date_souhaite ?? '—' }}</span>
                    </div>
                </div>
